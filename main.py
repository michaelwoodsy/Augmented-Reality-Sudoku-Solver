...
def ar_sudoku_solver(image):
    ...
    if sudoku_contour is not None:
        ...
        warped_image = warp_image(
            warped_image, sudoku_contour, image_width, image_height
        )

        # Record time taken to warp the image
        warp_time = time.time()
        print(
            "Time taken to detect the grid is {} seconds".format(warp_time - start_time)
        )
        ...
        horizontal_lines, vertical_lines = copy.deepcopy(
            warp_preprocessed_image
        ), copy.deepcopy(warp_preprocessed_image)
        ...
        if complete_grid is not None:
            ...
            if num_clues >= 17:
                ...
                resized_number_images = resize_number_images(
                    cleaned_number_images, model_dimension
                )
                ...
                print(
                    "Time taken to detect the sudoku numbers is {} seconds".format(
                        sudoku_time - warp_time
                    )
                )
                ...
                if validate_sudoku(checker_board):
                    ...
                    if type(solved_sudoku) is not bool:
                        ...
                        print(
                            "Time taken to solve sudoku is {} seconds".format(
                                solve_time - sudoku_time
                            )
                        )

                        # Overlay the solution to the sudoku on the warped image
                        overlay_warped_image = overlay_solution(
                            warped_image,
                            solved_sudoku,
                            checker_board,
                            model_dimension,
                        ...
                        final_solution = unwarp_image(
                            overlay_warped_image,
                            image,
                            sudoku_contour,
                            image_width,
                            image_height,
                        ...
                        print(
                            "Time taken to finish running is {} seconds".format(
